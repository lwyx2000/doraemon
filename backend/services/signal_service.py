...
def get_tracked(user_id: str) -> list[dict]:
    ...
    rows = db.fetchall(
        ...
        "biz_signal_subscriptions WHERE user_id = ? ORDER BY created_at DESC",
        [user_id],
    )
    out = []
    for r in rows:
...
def add_tracked(user_id: str, symbol: str, name: str | None, strategy_id: str, params: dict) -> dict:
    ...
    db = get_db()
    db.execute(
        "INSERT INTO biz_signal_subscriptions (id, user_id, symbol, name, strategy_id, params_json, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
def remove_tracked(user_id: str, rec_id: str) -> bool:
    ...
    db = get_db()
    db.execute(
        "DELETE FROM biz_signal_subscriptions WHERE id = ? AND user_id = ?",
        [rec_id, user_id],
    )
    return True
