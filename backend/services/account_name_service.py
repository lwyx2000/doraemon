...
def _load_names(user_id: str) -> list[dict]:
    ...
    rows = db.fetchall(
        ...
        "FROM biz_account_names WHERE fk_users = ? ORDER BY created_at",
        [user_id],
    )
    return [
...
def _save_name(user_id: str, item: dict) -> None:
    ...
    db.execute(
        "INSERT INTO biz_account_names (id, fk_users, name, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?)",
...
def delete_account_name(user_id: str, name_id: str) -> bool:
    ...
    db = get_db()
    row = db.fetchone(
        "SELECT count(*) FROM biz_account_names WHERE id = ? AND fk_users = ?",
        [name_id, user_id],
    )
