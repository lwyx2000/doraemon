...
def _load_accounts(user_id: str) -> list[dict]:
    ...
    rows = db.fetchall(
        ...
        "FROM biz_broker_accounts WHERE user_id = ? ORDER BY created_at",
        [user_id],
    )
    return [
...
def _save_account(user_id: str, item: dict) -> None:
    ...
    db.execute(
        "INSERT INTO biz_broker_accounts (id, user_id, broker, account, created_at, updated_at) "
        "VALUES (?, ?, ?, ?, ?, ?)",
...
def delete_broker_account(user_id: str, account_id: str) -> bool:
    ...
    db = get_db()
    row = db.fetchone(
        "SELECT count(*) FROM biz_broker_accounts WHERE id = ? AND user_id = ?",
        [account_id, user_id],
    )
