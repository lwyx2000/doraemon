...
def get_strategies(user_id: str) -> list[dict]:
    ...
    rows = db.fetchall(
        ...
        "FROM biz_strategies WHERE fk_user = ? ORDER BY created_at",
        [user_id],
    )
...
def create_strategy(
    ...
    db.execute(
        ...
        "(id, fk_user, name, target_asset, rules, sort_by, sort_order, limit_count, active, ai_tracking, created_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
def update_strategy(
    ...
    if sets:
        ...
        db.execute(
            f"UPDATE biz_strategies SET {', '.join(sets)} WHERE id = ? AND fk_user = ?",
            params,
        )
    # 重新读取
    row = db.fetchone(
        "SELECT id, name, target_asset, rules, sort_by, sort_order, limit_count, "
        "active, ai_tracking, created_at "
        "FROM biz_strategies WHERE id = ? AND fk_user = ?",
        [strategy_id, user_id],
    )
...
def delete_strategy(user_id: str, strategy_id: str) -> bool:
    ...
    db = get_db()
    row = db.fetchone(
        "SELECT count(*) FROM biz_strategies WHERE id = ? AND fk_user = ?",
        [strategy_id, user_id],
    )
